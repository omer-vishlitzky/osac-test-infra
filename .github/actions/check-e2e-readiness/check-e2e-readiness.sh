...
REQUIRE_E2E_READY="${REQUIRE_E2E_READY:-false}"
...
if [[ -z "${reason:-}" ]]; then
  if [[ "${REQUIRE_E2E_READY}" == "true" ]]; then
    reason="waiting: trusted /e2e-ready unlock is required"
  else
...
echo "::notice::${reason}"
if [[ "${REQUIRE_E2E_READY}" == "true" ]]; then
  echo "::notice::An organization member must unlock this PR with \`/e2e-ready\`."
else
