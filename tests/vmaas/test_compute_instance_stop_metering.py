...
def test_compute_instance_stop_metering(
    ...
    try:
        ...
        # Wait for in-flight heartbeats to drain. The heartbeat generator
        # queries the projection DB which lags behind Kafka publication,
        # so one more heartbeat sweep may fire after suspended.v1.
        time.sleep(HEARTBEAT_INTERVAL + 30)
        no_more_after = datetime.now(UTC).isoformat()
        metering.assert_no_events(
            "osac.resource.heartbeat.v1",
            resource_id=uuid,
            since=no_more_after,
            within=HEARTBEAT_INTERVAL + 30,
        )
    finally:
